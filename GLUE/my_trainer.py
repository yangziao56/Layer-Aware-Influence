...
class Trainer:
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    with context():
                        ...
                        np.savetxt('/home/user/workspace/llm/results/remove_list.csv', np.array(self.noise_list), delimiter=',', fmt='%d')
    ...
    def compute_loss_SPL(self, model, inputs, skip_remove=False, return_outputs=False, num_items_in_batch=None):
        ...
        val_input_all, val_output_grads = self.my_evaluate()
        ...
        # with torch.no_grad():
        output_grads = outputs.gradients.detach()
        ...
        # =============================== ours =================================
        input_dot_sum = None
        for module, layer_data in self.layer_outputs.items():

            input_train = layer_data['input'].view(num_samples, -1).detach().to(output_grads.device)
            input_val = val_input_all[module].detach().to(output_grads.device)
        ...
        val_output_grads = val_output_grads.detach()
        ghost_total = torch.mm(output_train_grads, val_output_grads.T)
        ...
        if len(indices) < num_samples and len(indices) > 0:
            ...
            print('=====', len(removed_list), '/', num_samples)
    ...
    def my_prediction_step(
        ...
        for module, layer_data in self.layer_outputs.items():
            ...
            input_train = layer_data['input'].view(num_samples, -1).detach()
            input_all[module] = torch.sum(input_train, dim=0, keepdim=True)

        output_grads = output_grads.view(num_samples, -1).detach()
        output_grads = torch.sum(output_grads, dim=0, keepdim=True)

        self.layer_outputs = {}

        return (input_all, output_grads, num_samples)
    ...
    def my_evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            cur_input_all, cur_output_grads, num_samples = self.my_prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            if output_grads is None:
                output_grads = cur_output_grads
                for key in cur_input_all.keys():
                    input_all[key] = cur_input_all[key]
            else:
                output_grads += cur_output_grads
                for key in cur_input_all.keys():
                    input_all[key] += cur_input_all[key]
            total_number += num_samples

            if is_torch_xla_available():
                xm.mark_step()

        output_grads = output_grads / total_number
        for key in input_all.keys():
            input_all[key] = input_all[key] / total_number

        del cur_input_all, cur_output_grads, inputs
        torch.cuda.empty_cache()


        return input_all, output_grads
    ...
    def my_evaluate(
        ...
        override = eval_dataset is not None
        eval_dataset = eval_dataset if override else self.eval_dataset
        if len(eval_dataset) > 2000:
            random_indices = random.sample(range(len(eval_dataset)), 2000)
        ...
        input_all, output_grads = self.my_evaluation_loop(
            eval_dataloader,
            description="Evaluation",
            prediction_loss_only=True if self.compute_metrics is None else None,
            ignore_keys=ignore_keys,
