...
class Trainer:
    ...
    def __init__(
        ...
        def backward_hook(module, grad_input, grad_output):
            if module in self.layer_outputs:
                ...
                self.layer_outputs[module]['grad_output'] = grad_output
    ...
    def compute_loss_SPL(self, model, inputs, skip_remove=False, return_outputs=False, num_items_in_batch=None):
        ...
        if len(indices) < num_samples and len(indices) > 0:
            ...
            del ghost_total, outputs, output_grads, input_train, input_dot, input_dot_sum, output_train_grads, loss, labels
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            return self.compute_loss_SPL(model, inputs, skip_remove=True), removed_list
        else:
            del ghost_total, outputs, model, inputs
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            return loss, []
