...
is_accelerate_available = getattr(hf_utils, "is_accelerate_available", lambda *a, **k: _module_available("accelerate"))
is_apex_available = getattr(hf_utils, "is_apex_available", lambda: False)
is_bitsandbytes_available = getattr(hf_utils, "is_bitsandbytes_available", lambda: False)
is_datasets_available = getattr(hf_utils, "is_datasets_available", lambda: _module_available("datasets"))
is_galore_torch_available = getattr(hf_utils, "is_galore_torch_available", lambda: False)
is_in_notebook = getattr(hf_utils, "is_in_notebook", lambda: False)
is_ipex_available = getattr(hf_utils, "is_ipex_available", lambda: False)
is_lomo_available = getattr(hf_utils, "is_lomo_available", lambda: False)
is_peft_available = getattr(hf_utils, "is_peft_available", lambda: _module_available("peft"))
is_safetensors_available = getattr(hf_utils, "is_safetensors_available", lambda: _module_available("safetensors"))
is_sagemaker_dp_enabled = getattr(hf_utils, "is_sagemaker_dp_enabled", lambda: False)
is_sagemaker_mp_enabled = getattr(hf_utils, "is_sagemaker_mp_enabled", lambda: False)
is_torch_compile_available = getattr(hf_utils, "is_torch_compile_available", lambda: False)
is_torch_mlu_available = getattr(hf_utils, "is_torch_mlu_available", lambda: False)
is_torch_mps_available = getattr(hf_utils, "is_torch_mps_available", lambda min_version=None: False)
is_torch_musa_available = getattr(hf_utils, "is_torch_musa_available", lambda: False)
is_torch_neuroncore_available = getattr(hf_utils, "is_torch_neuroncore_available", lambda: False)
is_torch_npu_available = getattr(hf_utils, "is_torch_npu_available", lambda: False)
is_torch_xla_available = getattr(hf_utils, "is_torch_xla_available", lambda: _module_available("torch_xla"))
is_torch_xpu_available = getattr(hf_utils, "is_torch_xpu_available", lambda: False)
is_torchao_available = getattr(hf_utils, "is_torchao_available", lambda: False)
...
if is_torch_xla_available():
    ...
    try:
        import torch_xla.runtime as xr  # pytype: disable=import-error
    except ImportError:  # pragma: no cover - legacy XLA installs
        xr = None
    ...
    if xr is not None:
        ...
        if not hasattr(xm, "get_local_ordinal"):
            xm.get_local_ordinal = xr.local_ordinal
else:
...
class Trainer:
    ...
    def __init__(
        ...
        self.args = args
        for _opt_attr, _default in (
            ("use_liger_kernel", False),
            ("average_tokens_across_devices", False),
        ):
            if not hasattr(self.args, _opt_attr):
                setattr(self.args, _opt_attr, _default)
        ...
        self.noise_list = []
        self._pending_shapley_scores: Optional[np.ndarray] = None
        self._pending_shapley_step: Optional[int] = None
        self._last_batch_ghost_scores: Optional[np.ndarray] = None
        self._last_batch_jiacheng_scores: Optional[np.ndarray] = None
        self._shapley_logging_interval = 100
        self._shapley_logging_start_step = 100
        self._shapley_csv_initialized = False
        self._shapley_output_path = Path(__file__).resolve().parent / "first_batch_scores.csv"
        ...
        def forward_hook(module, input, output):
            if isinstance(output, torch.Tensor) and output.requires_grad:
                output.retain_grad()
            self.layer_outputs[module] = {
                'input': input[0],
                'output': output if isinstance(output, torch.Tensor) else None,
            }

        def backward_hook(module, grad_input, grad_output):
            if module in self.layer_outputs:
                # layer_outputs[module]['grad_input'] = grad_input
                # grad_output is a tuple for nn.Linear; take the tensor part when available
                if grad_output and isinstance(grad_output[0], torch.Tensor):
                    self.layer_outputs[module]['grad_output'] = grad_output[0]
    ...
    def _inner_training_loop(
        ...
        self._pending_shapley_scores = None
        self._pending_shapley_step = None
        self._last_batch_ghost_scores = None
        self._last_batch_jiacheng_scores = None

        if self.is_world_process_zero():
            self._shapley_output_path.parent.mkdir(parents=True, exist_ok=True)
            with self._shapley_output_path.open("w", newline="") as csv_file:
                writer = csv.writer(csv_file)
                writer.writerow(["global_step", "shapley", "ghost_score", "jiacheng_score"])
            self._shapley_csv_initialized = True
        else:
            self._shapley_csv_initialized = False
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    target_step = self.state.global_step + 1
                    should_compute_shapley = (
                        self._shapley_logging_interval > 0
                        and do_sync_step
                        and target_step >= self._shapley_logging_start_step
                        and (target_step - self._shapley_logging_start_step)
                        % self._shapley_logging_interval
                        == 0
                    )

                    if should_compute_shapley:
                        num_shapley_samples = 1000
                        logger.warning(
                            "Shapley debug: computing at global_step=%s on rank %s",
                            target_step,
                            getattr(self.args, "process_index", "?"),
                        )
                        try:
                            shapley_scores = self._estimate_first_batch_shapley(
                                model,
                                inputs,
                                num_permutations=num_shapley_samples,
                            )
                            if shapley_scores is not None:
                                self._pending_shapley_scores = np.asarray(shapley_scores, dtype=np.float64)
                                self._pending_shapley_step = target_step
                                if self.is_world_process_zero():
                                    formatted_scores = ", ".join(
                                        f"{score:.6f}" for score in self._pending_shapley_scores.tolist()
                                    )
                                    logger.warning(
                                        "Shapley values (MC %d, step %d): [%s]",
                                        num_shapley_samples,
                                        target_step,
                                        formatted_scores,
                                    )
                                ...
                                logger.warning(
                                    "Shapley debug: estimation returned None on rank %s at global_step=%s.",
                                    getattr(self.args, "process_index", "?"),
                                    target_step,
                                )
                        except Exception as exc:
                            logger.warning(
                                "Failed to compute Shapley values at global_step=%s: %s",
                                target_step,
                                exc,
                            )
                    ...
                    with context():
                        tr_loss_step, removed_list = self.training_step(model, inputs, num_items_in_batch)
                        # print('====', removed_list, 'length:', len(inputs['input_ids']), 'batch_size:', self.args.per_device_train_batch_size, 'step:', step)
                        self.noise_list += [cur_idx + self.args.per_device_train_batch_size * self.num_gpus * step for cur_idx in removed_list]
                        remove_dir = os.path.join(self.args.output_dir, "results")
                        os.makedirs(remove_dir, exist_ok=True)
                        remove_path = os.path.join(remove_dir, "remove_list.csv")
                        np.savetxt(remove_path, np.array(self.noise_list), delimiter=',', fmt='%d')

                    if (
                        self._pending_shapley_scores is not None
                        and self._last_batch_ghost_scores is not None
                        and self._pending_shapley_step is not None
                        and self.is_world_process_zero()
                        and self._shapley_csv_initialized
                    ):
                        if self._last_batch_jiacheng_scores is None:
                            logger.warning(
                                "Shapley debug: Jiacheng scores unavailable; skipping CSV export at global_step=%s.",
                                self._pending_shapley_step,
                            )
                            self._pending_shapley_scores = None
                            self._pending_shapley_step = None
                            self._last_batch_ghost_scores = None
                            self._last_batch_jiacheng_scores = None
                            continue
                        shapley_arr = np.asarray(self._pending_shapley_scores, dtype=np.float64)
                        ghost_arr = np.asarray(self._last_batch_ghost_scores, dtype=np.float64)
                        jiacheng_arr = np.asarray(self._last_batch_jiacheng_scores, dtype=np.float64)
                        if (
                            shapley_arr.shape[0] != ghost_arr.shape[0]
                            or shapley_arr.shape[0] != jiacheng_arr.shape[0]
                        ):
                            logger.warning(
                                "Shapley debug: length mismatch (Shapley=%s, ghost=%s, Jiacheng=%s) at global_step=%s.",
                                shapley_arr.shape[0],
                                ghost_arr.shape[0],
                                jiacheng_arr.shape[0],
                                self._pending_shapley_step,
                            )
                        else:
                            rows = [
                                [
                                    int(self._pending_shapley_step),
                                    float(shapley_arr[idx]),
                                    float(ghost_arr[idx]),
                                    float(jiacheng_arr[idx]),
                                ]
                                for idx in range(shapley_arr.shape[0])
                            ]
                            with self._shapley_output_path.open("a", newline="") as csv_file:
                                writer = csv.writer(csv_file)
                                writer.writerows(rows)
                            logger.warning(
                                "Shapley debug: appended %s rows for global_step=%s to %s",
                                shapley_arr.shape[0],
                                self._pending_shapley_step,
                                self._shapley_output_path,
                            )
                        self._pending_shapley_scores = None
                        self._pending_shapley_step = None
                        self._last_batch_ghost_scores = None
                        self._last_batch_jiacheng_scores = None
                    elif self._pending_shapley_scores is not None and self._pending_shapley_step is not None:
                        self._pending_shapley_scores = None
                        self._pending_shapley_step = None
                        self._last_batch_ghost_scores = None
                        self._last_batch_jiacheng_scores = None
                    ...
                    if do_sync_step:
                        ...
                        if hasattr(self.callback_handler, "on_pre_optimizer_step"):
    ...
    def training_step(
        ...
        with self.compute_loss_context_manager():
            ...
            # removed_indices = []
            loss, removed_indices = self.compute_loss_SPL(model, inputs, num_items_in_batch=num_items_in_batch)
        ...
        if self.use_apex:
            ...
            if num_items_in_batch is None:
                return loss.detach() / self.args.gradient_accumulation_steps, removed_indices
            return loss.detach(), removed_indices

    def _estimate_first_batch_shapley(
        self,
        model: nn.Module,
        inputs: Dict[str, Union[torch.Tensor, Any]],
        ...
        for key, value in inputs.items():
            if isinstance(value, torch.Tensor):
                batch_inputs[key] = value.detach().clone()
            else:
                batch_inputs[key] = copy.deepcopy(value)

        labels = batch_inputs.get("labels")
        if labels is None:
            labels = batch_inputs.get("input_ids")
            if labels is None:
                logger.warning("Shapley debug: no labels or input_ids found in first batch.")
                return None
        ...
        try:
            ...
            if logits is None:
                logger.warning("Shapley debug: model outputs missing logits.")
                return None

            logger.warning(
                "Shapley debug: logits shape %s, labels shape %s", tuple(logits.size()), tuple(labels.size())
            )

            if logits.size(0) == 0 or logits.size(1) < 2:
                logger.warning(
                    "Shapley debug: logits shape %s invalid for token loss (batch or seq length too small).",
                    tuple(logits.size()),
                )
                return None
            ...
            if attention_mask is not None:
                shift_attention_mask = attention_mask[:, 1:].reshape(-1)
                valid_mask = valid_mask & (shift_attention_mask != 0)

            if not valid_mask.any():
                logger.warning("Shapley debug: valid token mask empty (all tokens masked).")
                per_sample_loss = np.zeros(batch_size, dtype=np.float32)
            else:
                logger.warning(
                    "Shapley debug: valid tokens %d of %d", int(valid_mask.sum().item()), len(valid_mask)
                )
                losses_flat = logits_flat.new_zeros(labels_flat.shape, dtype=logits_flat.dtype)
                selected_logits = logits_flat[valid_mask]
                selected_labels = labels_flat[valid_mask]
                losses_flat[valid_mask] = F.cross_entropy(
                    selected_logits,
                    selected_labels,
                    reduction="none",
                )

            loss_per_token = losses_flat.view(batch_size, -1)
            token_weights = valid_mask.view(batch_size, -1).to(loss_per_token.dtype)
            denom = token_weights.sum(dim=1).clamp_min(1.0)
            per_sample_loss = (loss_per_token.sum(dim=1) / denom).detach().cpu().numpy()
            logger.warning("Shapley debug: per-sample loss %s", per_sample_loss)
        finally:
            if was_training:
                model.train()

        if per_sample_loss is None or len(per_sample_loss) == 0:
            return None

        per_sample_loss = per_sample_loss.astype(np.float64, copy=False)

        rng_seed = getattr(self.args, "seed", None)
        rng = np.random.default_rng(rng_seed)
        shapley_scores = np.zeros_like(per_sample_loss, dtype=np.float64)

        for _ in range(num_permutations):
            ordering = rng.permutation(len(per_sample_loss))
            current_sum = 0.0
            current_count = 0
            prev_loss = 0.0
            for idx in ordering:
                current_sum += float(per_sample_loss[idx])
                current_count += 1
                current_loss = current_sum / current_count
                shapley_scores[idx] += current_loss - prev_loss
                prev_loss = current_loss

        shapley_scores /= float(num_permutations)
    ...
    def compute_loss_SPL(self, model, inputs, skip_remove=False, return_outputs=False, num_items_in_batch=None):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.
        ...
        output_grads = output_grads.detach()
        num_samples = output_grads.shape[0]
        if num_samples == 0 or not self.layer_outputs:
            del outputs, model, inputs
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
        ...
        # print('==================================', num_samples, len(self.layer_outputs.items()))
        ...
        input_dot_sum = None
        jiacheng_total = None
        for _, layer_data in self.layer_outputs.items():

            input_train = layer_data['input'].view(num_samples, -1).detach().to(output_grads.device)
            input_dot = torch.mm(input_train, input_train.T).detach()
            if input_dot_sum is None:
                input_dot_sum = input_dot
            else:
                input_dot_sum += input_dot

            grad_output_tensor = layer_data.get('grad_output')
            if grad_output_tensor is None:
                output_tensor = layer_data.get('output')
                if output_tensor is None or not isinstance(output_tensor, torch.Tensor):
                    continue
                try:
                    grad_output_tensor = torch.autograd.grad(
                        loss,
                        output_tensor,
                        retain_graph=True,
                        create_graph=False,
            ...
            grad_output_tensor = grad_output_tensor.detach().to(output_grads.device)
            if grad_output_tensor.shape[0] != num_samples:
                continue
            grad_output_flat = grad_output_tensor.view(num_samples, -1)
            grad_dot = torch.mm(grad_output_flat, grad_output_flat.T).detach()

            layer_contrib = (input_dot * grad_dot).detach()
            if jiacheng_total is None:
                jiacheng_total = layer_contrib
            else:
                jiacheng_total += layer_contrib

        input_dot_sum = input_dot_sum.detach()
        output_train_grads = output_grads.view(num_samples, -1).detach()
        ghost_total = torch.mm(output_train_grads, output_train_grads.T)
        ghost_total = (ghost_total * input_dot_sum).mean(dim=1)
        if jiacheng_total is not None:
            jiacheng_scores = jiacheng_total.mean(dim=1)
        else:
            jiacheng_scores = None
        if self._pending_shapley_scores is not None:
            try:
                self._last_batch_ghost_scores = ghost_total.detach().to("cpu").numpy()
                if jiacheng_scores is not None:
                    self._last_batch_jiacheng_scores = jiacheng_scores.detach().to("cpu").numpy()
                else:
                    self._last_batch_jiacheng_scores = None
            except Exception:
                self._last_batch_ghost_scores = None
        ...
        #     input_train = layer_data['input'].view(num_samples, -1).detach().to(output_grads.device)
        #     input_val = torch.mean(input_train, dim=0, keepdim=True).detach()
        #     input_dot = torch.mm(input_train, input_val.T).detach()

        #     output_train_grads = layer_data['grad_output'].detach().to(output_grads.device)
        #     output_train_grads = output_train_grads.view(num_samples, -1)
        #     output_val_grads = torch.mean(output_train_grads, dim=0, keepdim=True).detach()
        #     grads_dot = torch.mm(output_train_grads, output_val_grads.T).detach()
        ...
        indices = (ghost_total > 0).nonzero(as_tuple=True)[0].detach()
        if len(indices) < num_samples and len(indices) > 0:
            for k in inputs.keys():
                inputs[k] = inputs[k][indices].detach()
            removed_list = (ghost_total <= 0).nonzero(as_tuple=True)[0].detach()
            removed_list = removed_list.tolist()
            print('=====', len(removed_list), '/', num_samples, '---', removed_list, '~~~~~~~~~~~~', ghost_total)
            del ghost_total, outputs, output_grads, input_dot_sum, output_train_grads, loss, labels, jiacheng_total, jiacheng_scores
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            return self.compute_loss_SPL(model, inputs, skip_remove=True), removed_list
        else:
            del ghost_total, outputs, model, inputs, input_dot_sum, output_train_grads, jiacheng_total, jiacheng_scores
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            return loss, []
